...
/// 1. Prevenção de logs duplicados (verifica últimos 30 segundos)
/// 2. Melhor logging para debug de sincronização
/// 3. Verificação de envio de embeddings ao cadastrar facial
...
class _ReconhecimentoFacialScreenState extends State<ReconhecimentoFacialScreen> {
  ...
  Future<void> _carregarDados() async {
    ...
    try {
      ...
      print('📊 [ReconhecimentoFacial] ${alunos.length} alunos, ${logs.length} logs hoje');
    } catch (e) {
  ...
  Future<void> _iniciarReconhecimento() async {
    try {
      ...
      if (resultado != null) {
        // ✅ Aluno reconhecido
        print('✅ [Reconhecimento] Aluno identificado: ${resultado['nome']}');
        await _selecionarTipoAcesso(resultado);
      } else {
        // ❌ Não reconhecido - mostrar opções
        print('⚠️ [Reconhecimento] Nenhum aluno reconhecido');
  ...
  Future<void> _abrirBuscaManual() async {
    ...
    if (aluno != null) {
      print('🔍 [BuscaManual] Aluno selecionado: ${aluno['nome']}');
  ...
  Future<void> _selecionarTipoAcesso(Map<String, dynamic> aluno) async {
    ...
    if (tipo != null) {
      print('📍 [TipoAcesso] Selecionado: $tipo para ${aluno['nome']}');
      await _registrarPassagem(aluno, tipo);
    }
  }

  // ================================================================
  // ✅ REGISTRAR PASSAGEM - COM PREVENÇÃO DE DUPLICATAS
  // ================================================================
  Future<void> _registrarPassagem(Map<String, dynamic> aluno, String tipo) async {
    try {
      ...
      print('📝 [RegistrarPassagem] Iniciando registro para ${aluno['nome']} - $tipo');

      // ✅ VERIFICAR SE JÁ EXISTE LOG RECENTE (últimos 30 segundos)
      final logsRecentes = await _db.getLogsHoje();

      bool logDuplicado = false;
      for (final log in logsRecentes) {
        if (log['cpf'] == cpf && log['tipo'] == tipo) {
          final logTime = DateTime.parse(log['timestamp']);
          final diferencaSegundos = timestamp.difference(logTime).inSeconds.abs();

          if (diferencaSegundos < 30) {
            logDuplicado = true;
            print('⚠️ [RegistrarPassagem] LOG DUPLICADO DETECTADO! Última passagem há $diferencaSegundos segundos');
            break;
          }
        }
      }
      ...
      if (logDuplicado) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('⚠️ ${aluno['nome']} já foi registrado recentemente\nem: ${_getTipoLabel(tipo)}'),
      ...
      print('💾 [RegistrarPassagem] Log salvo no banco local');
      ...
      print('📤 [RegistrarPassagem] Log enfileirado para sincronização');

      // ✅ TENTAR SINCRONIZAR IMEDIATAMENTE
      final syncResult = await OfflineSyncService.instance.trySyncNow();
      print('🔄 [RegistrarPassagem] Tentativa de sync imediato: ${syncResult ? "SUCESSO" : "FALHOU"}');
      ...
      print('❌ [RegistrarPassagem] Erro: $e');
      if (Navigator.canPop(context)) Navigator.pop(context);
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () async {
              _mostrarProgresso('Sincronizando...');
              final result = await OfflineSyncService.instance.trySyncNow();
              if (Navigator.canPop(context)) Navigator.pop(context);

              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(result ? '✅ Sincronizado com sucesso' : '❌ Falha na sincronização'),
